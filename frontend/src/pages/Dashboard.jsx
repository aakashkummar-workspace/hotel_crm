...
import { Avatar, Metric, Pill, StatusPill } from '../components/primitives.jsx';
  ...
  return (
    <div className="page page-enter">
      ...
      <div className="stagger" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 28 }}>
        <Metric label="Today's revenue" value={fmtINRk(todayRevenue)} sub="Rooms, café, hall combined" trend={12} accent="var(--gold-2)" sparkData={revenue30.slice(-14).map(d => d.total)} />
        <Metric label="Monthly revenue" value={fmtINRk(monthRevenue)} sub="Month-to-date" trend={8} sparkData={revenue30.map(d => d.total)} />
        <Metric label="Total expenses" value={fmtINRk(totalExpenses)} sub={`${expenseBreakdown.length} categories tracked`} trend={-3} sparkData={[14, 18, 12, 16, 20, 15, 18, 22, 19, 24]} />
        <Metric label="Net profit" value={fmtINRk(monthRevenue - totalExpenses)} sub={`Margin ${monthRevenue ? Math.round(((monthRevenue - totalExpenses) / monthRevenue) * 100) : 0}%`} trend={14} accent="#9bc497" sparkData={[8, 10, 12, 9, 14, 18, 15, 20, 22, 24]} />
      </div>
